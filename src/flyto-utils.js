 ...
 */
export function flyToViewport(startProps, endProps, t, opts = {}) {
  ...
  const u =
    (w0 * ((Math.cosh(r0) * Math.tanh(r0 + rho * s) - Math.sinh(r0)) / rho2)) /
    u1;
...
function getFlyToTransitionParams(startProps, endProps, opts) {
  ...
  const startCenterXY = [startProps.target[0], startProps.target[1]];
  const startScale = zoomToScale(startZoom);
  const endZoom = endProps.zoom;
  const endCenterXY = [endProps.target[0], endProps.target[1]];
  ...
  const rho2 = rho * rho;
  const b0 =
    (w1 * w1 - w0 * w0 + rho2 * rho2 * _u1 * _u1) / (2 * w0 * rho2 * _u1);
  const b1 =
    (w1 * w1 - w0 * w0 - rho2 * rho2 * _u1 * _u1) / (2 * w1 * rho2 * _u1);
